...
const appID = "subtracotor"

var configFile string

func Run(ctx context.Context) error {
	cli.Init(appID, "subtractor service")
...
func start(parentCtx context.Context) {
	...
	app, err := New(ctx, cfg)
	if err != nil {
		log.Error("Unable to initialize subtractor", err)
	}
	...
	go func() {
		defer wg.Done()
		defer cancelFunc()
		log.Info("Starting subtractor")
		log.Error("subtractor stopped", app.Start())
	}()
